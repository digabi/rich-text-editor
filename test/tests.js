    ...
    describe('paste image', () => {
        before(done => {
            $('.answer1')
                ...
                $('.answer1 img:last').get(0).onload = () => { done() }
                $('.answer1').trigger({ type: 'paste', lol: 'bal', originalEvent: { clipboardData: { getData: () => { }}}})
        })
        it('saves pasted image', () => {
            expect($('.answer1 img:last')).to.have.attr('src').match(/\/screenshot/)
        })
    })
})
